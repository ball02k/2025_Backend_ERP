 ...
 *
 * Extracts key metadata from uploaded contract PDF documents using Tesseract.js.
 * Looks for: contract value, supplier name, dates, retention, payment terms, etc.
 *
 * Uses Tesseract.js - 100% free, opensource OCR with no external dependencies.
 */
...
class ContractOcrService {
  constructor() {
    console.log('📄 [ContractOCR] Initialized with Tesseract.js (free, opensource)');
  }
  ...
  async extractContractMetadata(fileBuffer, contractId) {
    try {
      ...
      // First try to extract text directly from PDF (faster, works for text-based PDFs)
      let rawText = '';
      try {
        const pdfData = await pdfParse(fileBuffer);
        rawText = pdfData.text;
        console.log(`✅ [ContractOCR] Extracted ${rawText.length} characters from PDF text layer`);
      } catch (pdfError) {
        console.log('⚠️ [ContractOCR] PDF text extraction failed, falling back to OCR:', pdfError.message);
      }

      // If no text extracted or very little, use OCR (for scanned PDFs)
      if (rawText.length < 100) {
        console.log('🔍 [ContractOCR] PDF appears to be scanned, using OCR...');

        // Convert PDF to images and OCR them
        // For now, we'll use pdf-parse text as fallback
        // In production, you'd want to convert PDF pages to images first
        rawText = await this.extractTextViaOCR(fileBuffer, contractId);
      }
  ...
  /**
   * Extract text using Tesseract OCR (for scanned PDFs/images)
   * This is a simplified version - for production, convert PDF pages to images first
   */
  async extractTextViaOCR(fileBuffer, contractId) {
    try {
      // Note: Tesseract works best with images, not PDFs directly
      // For a production system, you'd want to:
      // 1. Convert PDF pages to images using pdf-to-img or similar
      // 2. Run Tesseract on each image
      // 3. Combine results

      console.log('⚠️ [ContractOCR] OCR for scanned PDFs requires image conversion');
      console.log('⚠️ [ContractOCR] Using PDF text extraction as fallback');

      // Fallback: try harder with pdf-parse
      const pdfData = await pdfParse(fileBuffer, {
      ...
      console.error('❌ [ContractOCR] OCR extraction failed:', error);
      return '';
    }
  }
  ...
  async downloadFromLocalStorage(storageUrl) {
    try {
      // storageUrl is like "/uploads/contract-7-signed-1763985640989.pdf"
