function pathFor(type, ids = {}) {
  ...
  switch (type) {
    ...
    case 'budget': return projectId ? `/projects/${projectId}/budgets` : '/projects';
    case 'client': return `/clients/${id}`;
    case 'contact': return `/contacts/${id}`;
    case 'user': return `/users/${id}`;
    case 'supplier': return `/suppliers/${id}`;
    case 'po': return `/finance/pos/${id}`;
    case 'invoice': return projectId ? `/projects/${projectId}/finance/invoices/${id}` : `/finance/invoices/${id}`;
    case 'variation': return `/variations/${id}`;
    case 'afp': return projectId ? `/projects/${projectId}/finance/applications` : `/finance/afp/${id}`;
    case 'cvr': return projectId ? `/projects/${projectId}/cvr` : `/cvr`;
