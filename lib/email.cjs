...
async function throttleSmtpStart() {
  const minInterval = getMinSendIntervalMs();
  if (!minInterval) return;

  const now = Date.now();
  const waitMs = Math.max(0, lastSmtpStartAt + minInterval - now);
  if (waitMs > 0) await sleep(waitMs);
  lastSmtpStartAt = Date.now();
}
...
async function sendEmail(options) {
  ...
  // Local/dev/test modes should not wait behind the SMTP queue.
  if (process.env.SKIP_EMAIL === 'true' || !host) {
  ...
  const run = async () => {
    ...
    for (;;) {
      try {
        await throttleSmtpStart();
        return await sendEmailNow(options);
      } catch (err) {
        if (attempt >= maxRetries || !isRetryableSmtpError(err)) {
          throw err;
        }

        attempt += 1;
        const delayMs = getMinSendIntervalMs() * attempt;
        console.warn(`[email] Retryable SMTP failure, retrying in ${delayMs}ms (${attempt}/${maxRetries}):`, err.message);
        await sleep(delayMs);
      }
    }
  };

  emailQueue = emailQueue.then(run, run);
  return emailQueue;
}

async function sendEmailNow({ to, subject, text, from }) {
  const host = process.env.SMTP_HOST;
  ...
  // If SKIP_EMAIL is explicitly set to true, just log and return
  if (process.env.SKIP_EMAIL === 'true') {
    console.log('[email] SKIP_EMAIL=true - Email not sent:');
    console.log('  To:', to);
    ...
    if (linkMatch) {
      console.log('  🔗 INVITE LINK:', linkMatch[0]);
    }
    console.log('');
