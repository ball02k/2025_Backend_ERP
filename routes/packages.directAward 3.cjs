...
router.post('/packages/:id/direct-award', requireProjectMember, async (req, res) => {
  ...
  try {
    ...
    // Fallback to budgetLineItem if PackageLineItem doesn't exist or is empty
    let budgetLines = [];
    if (!packageLineItems.length) {
      try {
        budgetLines = await prisma.budgetLineItem.findMany({
          where: { packageId: pkg.id },
          orderBy: { id: 'asc' },
        });
      } catch (err) {
        console.warn('[direct-award] budgetLineItem lookup failed', err?.message);
      }
    }
