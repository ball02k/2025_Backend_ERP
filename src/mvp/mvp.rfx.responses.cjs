...
router.post('/mvp/rfx/:rfxId/upload-response', async (req, res, next) => {
  try {
    ...
    const pickTable = (name) => {
      ...
      if (!m) return [];
      return [...m[1].matchAll(/<Row>([\s\S]*?)<\/Row>/g)].map((r) => [...r[1].matchAll(/<Data[^>]*>([\s\S]*?)<\/Data>/g)].map((c) => c[1]));
    };
    const pricingRows = pickTable('Pricing');
    const questionsRows = pickTable('Questions');
    const pricing = pricingRows.slice(1).filter(Boolean).map((r) => ({ supplierId: Number(r[0] || supplierId), item: r[1], description: r[2], qty: r[3] ? Number(r[3]) : null, unit: r[4] || null, rate: r[5] ? Number(r[5]) : null, total: r[6] ? Number(r[6]) : null })).filter((x) => x.item || x.description);
    const answers = questionsRows.slice(1).filter(Boolean).map((r) => ({ supplierId: Number(r[0] || supplierId), question: r[1], response: r[2] || '' })).filter((x) => x.question);
    const sub = await prisma.rFxSubmission.upsert({ where: { tenantId_rfxId_supplierId: { tenantId, rfxId, supplierId: Number(supplierId) } }, update: { pricing, answers }, create: { tenantId, rfxId, supplierId: Number(supplierId), pricing, answers } });
